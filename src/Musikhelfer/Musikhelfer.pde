...
// Set the Values for the tuner volume bar.
float tuneVolumeX = 360;
float tuneVolumeY = 645;
float tuneVolumeMin = 160;
float tuneVolumeMax = 560;
float tuneVolume = 0.5;
boolean volumeBar = false;
...
void mousePressed() {
  // Kai Yun Chao | 3B
  // Constrains the volume of the Sound files.


  // Detects and opporates tuneButtons activation.
  for (int i = 0; i<tuneButtons.length; i++) {
    if (tuneButtons[i].over) {
      // pitch.amp(tuneVolume);

      play(tuneButtons[i].disVal);
    }
  }
  //matches the noteVal to disVal value of the button class.
  for (int i = 0; i<tuneButtons.length; i++) {
  ...
  // Assigns variable B.
  float b = dist(mouseX, mouseY, tuneVolumeX, tuneVolumeY);
  // Where the mouse is/ volume bar.
  if (b < 15) {
    volumeBar = true;
  }
  //Part of my attempt to connect the bar to Sound/amp control.
  //}
  //if ( volumeBar) {
  //  for (int i = 0; i < tuneButtons.length; i++) {
  //    pitch.amp(tuneVolume);
  //  }
  //}
  //Activates boolean for color change in void tunerMode() {}.
  if (dist(mouseX, mouseY, tuneVolumeX, 600) < 20) {
    volumeBar = true;
  }
  // Stops previous notes attempt.
  //for (int i = 0; i < tuneButtons.length; i++) {
  ...
  //  if (pitch == null) pitch = "";
  //  pitch = pitch.trim();
  //  if (pitch.equals("A4.mp3")) next = pitchA4;
  //  else if (pitch.equals("As4.mp3")) next = pitchAs4;
  //  else if (pitch.equals("B4.mp3")) next = pitchB4;
  //  else if (pitch.equals("C4.mp3")) next = pitchC4;
  //  else if (pitch.equals("Cs4.mp3")) next = pitchCs4;
  //  else if (pitch.equals("D4.mp3")) next = pitchD4;
  //  else if (pitch.equals("Ds4.mp3")) next = pitchDs4;
  //  else if (pitch.equals("E4.mp3")) next = pitchE4;
  //  else if (pitch.equals("F4.mp3")) next = pitchF4;
...
void mouseDragged() {
  //Kai Yun Chao, 3B
  if (volumeBar) {
    // Constrains the volume bar knob coordinate/location, also part of my attept to connect the sound/amp control.
    tuneVolumeX = constrain(mouseX, tuneVolumeMin, tuneVolumeMax);
    tuneVolume = map(tuneVolumeX, tuneVolumeMin, tuneVolumeMax, 0, 1);
  }
}
...
void tunerMode() {
  ...
  stroke(0);
  strokeWeight(1.5);
  line(tuneVolumeMin, tuneVolumeY, tuneVolumeMax, tuneVolumeY);
  // Changes knob color.
  if (volumeBar) {
    fill(#7FA3E0);
  } else {
  ...
  stroke(1);
  //volumeBar (Bar)
  rect(tuneVolumeX, tuneVolumeY, 30, 30, 9);
