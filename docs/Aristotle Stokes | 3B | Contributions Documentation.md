# Aristotle Stokes | 3B | Contributions Documentation

## Outline of Application Contributions
### Coded the MetroMode Method
  * MetroVal
  * Timing inbetween ticks
  * The metronome display
  * The metronome buttons
### Contributed to Creating the Start Screen
  * Created the logo
  * Created the gradient of the left sidebar

## Images of Application Contributions
Metronome Screen:

![Metronome Screen](https://github.com/Kai535813/Musikhelfer/blob/main/images/Metronome%20Screen.png?raw=true)

Start Screen:

![Start Screen](https://github.com/Kai535813/Musikhelfer/blob/main/images/Musik%20Helfer%20Start%20Screen.png?raw=true)
